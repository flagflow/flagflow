...
import type { ConfigService, LogService } from '$lib/server/services';
import {
	PersistentSchema,
...
import { InMemoryPersistentEngine } from './InMemoryPersistentEngine';

type MockPersistentServiceParameters = {
	configService: ConfigService;
	logService: LogService;
};

export const MockPersistentService = ({
	logService
}: Pick<MockPersistentServiceParameters, 'logService'>) => {
	const logger = logService('persistence');
	const engine = new InMemoryPersistentEngine();
	...
	return {
		...
		// Expose engine for testing utilities
		__testEngine: engine
	};
};
