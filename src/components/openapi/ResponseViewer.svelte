...
<Card class={`response-viewer ${aClass}`}>
	<Heading class="mb-4 flex items-center gap-2 text-base font-semibold" tag="h4">
	...
	{#if Object.keys(response.headers).length > 0}
		<div class="mb-4">
			...
			<div class="max-h-32 overflow-y-auto rounded-lg border bg-gray-50 p-3 font-mono text-sm">
	...
	<div>
		<div class="mb-2 flex items-center justify-between">
			<span class="flex items-center gap-2 text-sm font-medium text-gray-700">
				<Icon id={isJsonResponse ? 'object' : 'string'} size={16} />
				Response Body
				{#if response.json}
		...
		{#if response.body}
			<div class="max-h-96 overflow-auto rounded-lg border bg-gray-50 p-4 font-mono text-sm">
				{#if response.json}
		...
		{:else}
			<div class="rounded-lg border bg-gray-50 p-4 text-center text-gray-500 italic">
				No response body
			</div>
		{/if}
	</div>
	...
	<div class="mt-4 flex items-center justify-between text-xs text-gray-500">
		...
		{#if response.timing}
			<span>Request completed in {response.timing}ms</span>
		{/if}
	</div>
</Card>
