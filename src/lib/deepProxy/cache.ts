...
const getCache = <M extends WeakMap<any, any>, K>(map: M, key: K, Constructor: any) =>
	map.get(key) || map.set(key, new Constructor()).get(key);

const getKey = (path: string[]): string => path.filter((p) => typeof p !== 'symbol').join('​,');
...
export const getFromCache = <T extends TTarget>(
	...
	cache.proxies.get(cache.traps.get(root)?.get(target)?.get(getKey(path)) as TTraps) as unknown as
		| TProxy<T>
		| undefined;
