...
const parseParameters = <T extends TTarget>(
	trapName: TTrapName,
	parameters: [T, ...any[]]
): {
	target: T;
	...
	let target, name, value, receiver, arguments_, descriptor, thisValue, prototype;
    ...
    case 'get':
      ...
      break
    case 'set':
      [target, name, value, receiver] = parameters
      break
    case 'deleteProperty':
    case 'defineProperty':
    ...
    case 'getOwnPropertyDescriptor':
      ...
      break
    case 'apply':
      [target, thisValue, arguments_] = parameters
      break
    case 'construct':
      [target, arguments_] = parameters
      break
    case 'setPrototypeOf':
...
const createHandlerContext = <T extends TTarget>(
	...
	const { target, name, val, receiver, args, descriptor, thisValue, prototype } = parseParameters(
		trapName,
		parameters
	);
...
const trap = function <T extends TTarget>(this: TTrapContext, ...parameters: [T, ...any[]]) {
...
export const DeepProxy = class<T extends TTarget> {
	constructor(target: T, handler?: TProxyHandler, path?: string[], root?: TTarget) {
